...
`better-sqlite3` 是 Node 原生 C++ 插件，编译产物 `.node` 文件**系统、Node 版本强绑定**：

- Windows/macOS 本地编译的二进制不能在 Linux Docker 运行
- Node 大版本不同 ABI 不匹配，直接报 `Could not locate the bindings file`
- pnpm11 不再识别 `onlyBuiltDependencies`，不会自动重编译原生模块
